...
The following zones are grouped together to form the three major reef zone of interest:  
 ...
 - Fore Reef: Fore reef, Bank/Shelf, Bank/Shelf Escarpment, and Reef Crest      
...
zones <- coral_map %>% 
  ...
  mutate(zone = as.character(zone),
         ...
         zone = ifelse(zone == "Reef Crest", "Fore Reef", zone),
...
st_write(zones, file.path(dir_goal, "int/coral_zones.shp"), driver = 'ESRI Shapefile', append=FALSE)
...
coral_status <- zone_n %>% 
  ...
  mutate(n_total = case_when(zone == "Back Reef" ~ 126,
         ...
         year = 2020,
         habitat = "coral") %>%
  dplyr::select(region_id, habitat, zone, year, status)

# Save to output for use in coastal protection data prep
write_csv(coral_status, file.path(dir_goal, "output/coral_status.csv"))

## Make a dataframe to send to Alex/Nick 
status_back <- coral_status$status[1]
status_fore <- coral_status$status[2]
status_lag  <- coral_status$status[3]
...
exp_score <- zone_n %>% 
  ...
  mutate(n_total = case_when(zone == "Back Reef" ~ 126,
                             zone == "Fore Reef" ~ 169,
                             zone == "Lagoon" ~ 146),
         score = case_when(zone == "Back Reef" ~ status_back,
                           zone == "Fore Reef" ~ status_fore,
                           zone == "Lagoon" ~ status_lag))
```
