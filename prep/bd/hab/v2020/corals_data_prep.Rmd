...
Reference points for coral cover zones are:   
...
Find the extent (km^2^) of entire coral reef habitat. This extent is used in creating weights for pressures and resiliences.  
...
zone_n <- zones %>%
  group_by(zone, cover_cat, p_coral_cv) %>%
  summarize(n_cover = n()) %>% 
  mutate(ref = case_when(zone == "Back Reef" ~ ref_back,
                         zone == "Reef Crest" ~ ref_crest,
                         zone == "Fore Reef West" ~ ref_fore,
...
zone_areas <- zones %>% 
  group_by(zone) %>% 
  summarize(area_km = sum(area) * 0.000001)
...
coral_status <- zone_n %>% 
  ...
  mutate(n_total = case_when(zone == "Back Reef" ~ 126,
                             zone == "Reef Crest" ~ 12,
                             zone == "Fore Reef West" ~ 65,
                             zone == "Fore Reef East" ~ 92,
                             zone == "Lagoon" ~ 146),
         status = n_ref / n_total,
         region_id = 1, 
         year = 2020,
         habitat = "coral") %>% 
  left_join(zone_areas, by = "zone") %>% 
  dplyr::select(region_id, habitat, year, zone, area_km, status)

# Save to output for use in coastal protection data prep
write_csv(coral_status, file.path(dir_goal, "output/coral_status.csv"))
```