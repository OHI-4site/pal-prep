...
area_2001 <- read_csv(file.path(dir_goal, "int/trend_area_2001.csv"))
...
Each year will be given a status (0-1). The year's status will be calculated the same way the rainforest status was calculated in `rainforest_data_prep.Rmd`.  
...
```{r calculate-trend-status, eval=F}

areas     <- read_csv(file.path(dir_goal, "int/combined_trend_area.csv")) 
...
# Find the trend 
rainforest_trend <- areas %>% 
  mutate(potential_area = potential_forest_area,
         trend = area_forest / potential_forest_area) %>% 
...
write_csv(rainforest_trend, file.path(dir_scores, 'layers/rainforest_trend.csv'))
```
