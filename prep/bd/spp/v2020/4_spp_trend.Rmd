...
cat_lookup <- read_csv(file.path(dir_goal, "risk_code_lookup.csv")) # match old categories to today's categories
...
trend_na <- spp_trends %>% 
  filter(is.na(trend_score)) # 292 NA values - need to gapfill using the IUCN population trend
...
trend_na <- spp_trend_scores %>% 
  filter(is.na(trend_score)) # still 238 Nas
```
