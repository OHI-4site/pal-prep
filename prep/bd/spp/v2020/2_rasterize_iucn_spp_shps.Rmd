---
title: 'OHI+ Palmyra 2020: 2. Rasterize IUCN Species Maps'
...
``` {r rasterize_clip_save, eval=F}

maps_to_rasterize <- read_csv(file.path(dir_git, "prep/bd/spp/v2020/int/1_iucn_spp_shp_filepaths.csv")) %>%
  select(-X1) %>%
  mutate(shp_file = str_replace(dbf_file, 'dbf$', 'shp'))

### rast_base for cell IDs
rast_base <- raster(file.path(dir_git, "spatial/rasters/ocean_ras_50nm.tif")) %>%
