...
foreach(file = oa_rast_annual) %dopar% {
  ...
  rast_reproj <- projectRaster(from=r, to=three_nm, crs=us_alb, method="ngb", over=FALSE) %>% 
    raster::resample(three_nm, method="ngb", filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers/oa_%s.tif", dir_anx, yr), overwrite = TRUE) %>% 
    mask(three_nm, filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_coastal_%s.tif", dir_anx, yr), overwrite = TRUE) %>% 
    raster::crop(usalb_ext3) %>%
    writeRaster(., filename = paste0(file.path(dir_anx), '/dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_', yr, '.tif'), overwrite=T)
    
    # crop(three_nm, filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_%s.tif", dir_anx, yr), overwrite = TRUE)
  
  # rast_mask <- rast_reproj %>% 
  #   mask(three_nm, filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_coastal_%s.tif", dir_anx, yr), overwrite = TRUE) 
  #   
  # rast_crop <- crop(x = rast_mask, y = extent(three_nm), filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_%s.tif", dir_anx, yr), overwrite = TRUE)
  # 
    # crop(usalb_ext3) %>%
    #       writeRaster(., filename = paste0(file.path(dir_anx), '/dataprep/prs_oa/output/oa_rasts_us_albers/oa_', yr, '.tif'), overwrite=T)
    # 
  #   mask(three_nm, filename = sprintf("%s/dataprep/prs_oa/output/oa_rasts_us_albers/oa_li_", dir_anx, yr, '.tif'), overwrite = TRUE)
  # 
...
plot(raster(file.path(dir_anx, "dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_coastal_2020.tif")))

plot(raster(file.path(dir_anx, "dataprep/prs_oa/output/oa_rasts_us_albers/oa_2020.tif")))

plot(raster(file.path(dir_anx, "dataprep/prs_oa/output/oa_rasts_us_albers_coastal/oa_2020.tif")))
```
