...
dir_anx   <- file.path(dir_M, 'git-annex/foursite/palmyra') # Path to the NCEAS server where the bigger files are kept/saved
...
foreach (i = seq(1,708, by=12)) %dopar% {
  ...
  yr_mean = raw[[i:j]] %>%
            ...
            rotate() %>%
            projectRaster(ocean_rast3_100) %>% # Reproject to us_alb ocean raster
