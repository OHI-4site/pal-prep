...
This ocean acidification (OA) pressure layer preparation script uses the global ocean acidification data and finds an average ocean acidification pressure for Palmyra Atoll for each year from 1958 - 2020. 
...
Data for ocean acidification has been processed for the global assessment to give a value 0-1 for each year from 1958 - 2020. Information on how this data is processed to obtain 0-1 scores can be found [here](). 
...
foreach(file = oa_rast_annual) %dopar% {
  ...
  rast_reproj <- projectRaster(r, ocean_rast3_100, method = 'ngb') %>% 
          crop(usalb_ext3) %>%
...
write_csv(oa_data, file.path(dir_scores, "layers/cc_oa.csv"))
```
...
plot(raster(file.path(dir_anx, 'dataprep/prs_oa/output/oa_interpolated_cells.tif')), col=cols, box=F, axes=F, main='Interpolated cells')
