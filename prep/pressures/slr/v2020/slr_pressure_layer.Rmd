...
This sea level rise (SLR) pressure layer preparation script does the following for newly available SLR data:
...
- Resamples raster to ~ 1km^2^ and reprojects to Molleweide
...
dir_git       <- '~/github/pal-prep' # Path to the pal-prep github directory
dir_prs       <- '~/github/pal-prep/prep/pressures/slr/v2020'
# dir_anx       <- file.path(dir_M, 'git-annex/foursite/palmyra/dataprep') # Path to the NCEAS server where the bigger files are kept/saved
dir_prs_slr   <- sprintf("%s/git-annex/foursite/palmyra/dataprep/prs_slr", dir_M)
...
three_nm <- raster(file.path(dir_git, "spatial/rasters/ocean_ras3nm_100m.tif"))
 ...
 # file = annual_means[26]
  ...
  rast_data <- raster(file) %>%
    ...
    raster::resample(three_nm, method = "ngb", 
             filename = sprintf("%s/int/msla_annual_us_albers/mlsa_annual_us_albers_%s.tif", 
                                dir_prs_slr, yr), overwrite = TRUE) %>% 
    mask(three_nm, filename = sprintf("%s/int/msla_annual_us_albers_coastal/msla_annual_us_albers_coastal_%s.tif", dir_prs_slr, yr), overwrite = TRUE)
}
plot(raster(file.path(dir_prs_slr, "int/msla_annual_us_albers_coastal/msla_annual_us_albers_coastal_2010.tif")))
...
We are using a reference point 1.1 times larger than the maximum value in Palmyra from 1993-2019. The current maximum value is unlikely to be the future max, and setting this reference point allows for an increase in sea level rise pressure into the future.

```{r reference_point}

coastal_rasts <- list.files(file.path(dir_prs_slr, "int/msla_annual_us_albers_coastal"), pattern = "tif", full.names = TRUE)
...
foreach(file = coastal_rasts) %dopar% { # file = coastal_rasts[10]
  yr <- str_sub(file, -8,-5)
    ...
    calc(fun = function(x){ifelse(x > ref, 1, x/ref)}, # Set equal to 1 if greater than ref, otherwise scale
         filename = sprintf("%s/output/msla_rescaled/slr_%s.tif", dir_prs_slr, yr), overwrite = TRUE)
}
...
coastal_rasts <- list.files(file.path(dir_prs_slr, 'output/msla_rescaled'), full.names = T) %>%
                           stack()
...
pressure_stack <- lapply(list.files(file.path(dir_prs_slr, 'output/msla_rescaled'), full.names=T), raster) %>%
  brick()
...
pal_regionstats <- cellStats(pressure_stack, mean)%>% 
  ...
  select(-file_name) %>% 
  mutate(rgn_id = 12, # Change to Palmyra region - what rgn_id is it?
         rgn_name = "Northeast",
         state_abv = NA,
         state_name = NA,
         state = "Northeast",
...
slr_data <- rbind(slr_data_int, pal_regionstats) %>% 
  select(rgn_id, rgn_name, year, pressure_score) %>% 
  filter(year >= 2005 & year <= 2019)
write.csv(slr_data, file.path("~/github/pal-scores/region/layers/cc_slr.csv"), row.names=FALSE)
...
last_year <- slr_data%>%
  filter(year==2019,
         rgn_id != 12) # Change to Palmyra region ID
