...
The status will be found for each contaminant and then the average of all the contaminants will be used for the status score. At each site (s), the status of contamination is found by dividing the observed concentration of the contaminant by the reference level concentration, as in the following equation:   

$status_{c_{n},s}~=~(conc_n~/~ref_n)$   

Where $status_{c_{n},s}$ is the status of each contaminant $c_n$ at site $s$. The $conc_n$ is the observed concentration of the contaminant ($c_n$) and the $ref_n$ is the reference level concentration of each contaminant ($c_n$).   

The status for each contaminant is then found by averaging the status of the contaminant at each site. 

The overall status is the average of the status of each contaminant. Higher scores should be equated with lower levels of contamination, so the inverse is taken of the overall status (1 - overall status).  
...
rescale_value <- quantile(c_s_status$cs_status, probs = 0.9999)
