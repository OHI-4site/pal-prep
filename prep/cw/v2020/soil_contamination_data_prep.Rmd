...
contam_status <- contam_refs %>% 
  mutate(c_status = ifelse(conc > lwr_lmt, 0.5, (conc / lwr_lmt) / 2), 
         ...
         region_id = 1) %>% 
  dplyr::select(region_id, site, year, contam, status = c_status) 
...
areas     <- read_csv(file.path(dir_goal, "int/site_areas.csv"))

write_csv(sc_status, file.path(dir_scores, "layers/cw_contamination_status.csv"))
write_csv(areas, file.path(dir_scores, "layers/cw_contamination_extent.csv"))
```
