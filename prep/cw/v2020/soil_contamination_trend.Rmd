...
The current status for soil contamination will be found by dividing the observed concentration by the reference point for each contaminant at each site. Values are rescaled 0-1 using two orders of magnitude as the upper limit. Any value exceeding two orders of magnitude, or 100x over the threshold is assigned a value of 1.  

A status is then found for each contaminant in each year by averaging all the site scores. An overall status score is found for each year by averaging the status score for each of the 9 contaminants. This gives higher scores to higher levels of contamination. This will be used for the soil contamination pressure layer. The CW status score will be the inverse, found by 1 - status.   
...
site_status <- contam_refs %>% 
  ...
  mutate(s_status_resc = ifelse(s_status > 100, 1, s_status/100)) %>% 
  dplyr::select(site, year, soil_type, contam, s_status = s_status_resc, geometry)

# Find average status for each contaminant 
contam_status <- site_status %>% 
  group_by(contam, year) %>% 
  summarize(c_status = mean(s_status))

# Find overall status for each year 
status <- contam_status %>% 
  group_by(year) %>% 
  summarize(status = 1 - mean(c_status)) %>% 
  mutate(region_id = 1) %>% 
  dplyr::select(region_id, year, status)

# Save as dataframe 
sc_status <- as.data.frame(status) %>% 
