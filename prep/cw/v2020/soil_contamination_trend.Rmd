...
# Categorize offshore and inland 
sites <- usgs %>%
...
# Tidy format where contaminant observation is a single row
usgs_tidy <- pivot_longer(usgs_sites,
                          cols = pb:p,
                          names_to = "contaminant", values_to = "concentration") %>% 
  group_by(site, year, contaminant) %>% 
  mutate(concentration = mean(concentration)) %>% # Avg concentrations if study was repeated twice in same year
  ungroup() %>% 
  unique() %>% 
...
The current status for soil contamination will be found by dividing the observed concentration by the reference point for each contaminant at each site. 
...
site_status <- contam_refs %>% 
  ...
  arrange(-s_status) # up to 900x greater than the ref level.... 
```
