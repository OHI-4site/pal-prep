...
We will need this full list for the trend calculation in the toolbox. 

```{r save-to-toolbox}
dir_scores <- '~/github/pal-scores/region'

write_csv(ico_scinames, file.path(dir_scores, 'layers/ico_spp_scores.csv'))
```
...
ico_status <- ico_spp_scores %>%
  ...
  filter(year != 2000) %>% 
  summarize(status = mean(cat_score, na.rm = TRUE)) %>%
  ungroup()
```
