...
# select data
total_area <-
  rbind(rgn_area_inland1km,
        rgn_area_offshore3nm) %>% #total offshore/inland area 
...
# Join inland and offshore and find total protected area 
lsp_data <- full_join(offshore, inland, by = c("region_id", "year")) %>% 
  dplyr::select(-scenario_year.y) %>% 
  dplyr::rename("scenario_year" = "scenario_year.x") %>% 
...
Save this output to the toolbox as 'lsp_status.csv'
```{r save-to-toolbox}
dir_scores <- '~/github/pal-scores/region'

write_csv(status_data, file.path(dir_scores, 'layers/lsp_status.csv'))
```
