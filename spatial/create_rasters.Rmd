...
r_50nm <- fasterize(pal_boundary_alb, base_50nm, field = "rgn_id")
...
View one of Jamie's raster to see why mine keeps giving -1 and NA values:
```{r compare-ne-raster}
ne_git <- '~/github/ne-prep'

ne_rast <- raster(file.path(ne_git, 'spatial/ocean_rasters/ne_rgns_rast.tif')) %>% 
  projectRaster(crs = us_alb)

rgns <- st_read(file.path(ne_git, 'spatial/shapefiles/ne_ohi_rgns.shp'))

ocean_ne <- raster(file.path(ne_git, 'spatial/ocean_rasters/ocean_ne.tif'))
...
mapview(ocean_ne)
mapview(r)
# Looks like having the NA is okay, but the different regions should have values, in this case it's all one region so the value should just be 1
```
